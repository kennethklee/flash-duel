...
Game.prototype.start = function(callback) {
    ...
    var next = function(action) {
        // Last player should get cards
        var lastPlayer = self.players[index];

        if (lastPlayer) {
            var drawCardCount = 5 - lastPlayer.hand.cards.length;
            debug('Dealing ' + drawCardCount);
            self.deck.deal(drawCardCount, [lastPlayer.hand]);
        }
        ...
        if (player.state === Player.states.RETREATING) {
            process.nextTick(function() {
