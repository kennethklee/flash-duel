...
Action.prototype.attack = function(cardIndices, player) {
    ...
    if (allMatch && player.peice.position === attackPosition) {
        var defend = new Action.Defend(this, player, Action.attackTypes.NONRETREATABLE, cards);

        debug('Player ' + this.player.name + ' attacks ' + player.name + ' with ' + cards[0]);
...
Action.prototype.dashingStrike = function(moveCardIndex, attackCardIndices, player) {
    var self = this,
        moveCard = this.player.hand.drawAt(moveCardIndex)[0];
        attackCards = this.player.hand.drawAt(attackCardIndices),
        allMatch = attackCards.reduce(function(result, card) {result = result && card === attackCards[0]}, true),
        attackPosition = this.player.peice.position + ((card + cards[0]) * this.player.peice.direction);

    if (allMatch && player.peice.position === attackPosition) {
        var defend = new Game.Defend(this, player, Action.attackTypes.NORMAL, cards);
